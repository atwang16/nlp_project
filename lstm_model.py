...
class RNN(nn.Module):
    ...
    def forward(self, feature_tensor, hidden, state):
        output, hc_n = self.rnn(feature_tensor, (hidden, state))
...
def train(rnn, criterion, optimizer, train_data, question_data, hidden_size, num_layers, batch_size):
	...
	for i in range(num_samples / outer_batch_size):
		...
		input_batch = train_data[:outer_batch_size] # to test if weights and loss change
		X_scores = []
